...
- [ ] **UI 화면 확인 미완** — 로그인이 필요해 카드·결과 페이지·진입 페이지의 실제
      렌더링과 폴링 동작은 육안 확인하지 못했다. 상태 분기는 `CommentAnalysis.logic`
      테스트 17건이 덮지만 렌더링 자체는 자동 검증 대상이 아니다
...
2. **분류 배치가 전부 실패해도 환급되지 않는다.** `classify_comments`가 실패를 흡수해
